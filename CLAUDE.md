
<!-- MMY-INJECT:START -->
## 模型信息（由 MMYCodeSwitch-API 自动管理）

> 当前使用的模型为 **glm-5**，通过 **阿里云百炼-glm5** 提供服务。
> 此段内容由 `<!-- MMY-INJECT:START -->` 和 `<!-- MMY-INJECT:END -->` 包裹，
> 切换供应商或解绑项目时会自动更新，请勿手动编辑此区域。

<!-- MMY-INJECT:END -->---

## Blender 项目特有规范

### UI 挂载位置原则

功能 UI 应挂载在"最方便使用的地方"，避免冗余副本：

| 功能类型 | 推荐位置 | 示例 |
|----------|----------|------|
| 文件操作 | 文件浏览器 Header | 后缀保存 |
| 材质/节点操作 | Shader Editor 右键菜单/Header | 颜色空间切换 |
| 视图操作 | 3D 视图 Header | 相机焦距 |
| 通用工具 | N 面板页签 | 资产创建、缝合边 |

**原则**：原生位置优先，页签面板只放需要集中管理的工具，不在两处重复。

### EnumProperty 中文显示规范

Blender 的 `EnumProperty` 在处理中文时有三个坑点：

| 问题 | 解决方案 |
|------|----------|
| identifier 只接受 ASCII | 中文用 UTF-8 hex 编码：`'c' + name.encode('utf-8').hex()` |
| description 字段中文乱码（Windows） | description 留空，不要传入含中文的字符串 |
| items 列表被 GC 回收导致乱码 | 使用**模块级缓存列表**，回调函数返回同一个对象 |

**正确写法**：
```python
_ITEMS_CACHE = []  # 模块级缓存（关键）

def get_items(self, context):
    _ITEMS_CACHE.clear()
    for name in ["选项一", "选项二"]:
        safe_id = 'c' + name.encode('utf-8').hex()
        _ITEMS_CACHE.append((safe_id, name, ""))  # description 留空
    return _ITEMS_CACHE  # 返回同一个列表对象
```

详见：`开发文档/EnumProperty中文显示修复记录.md`

---

## 开发流程规范

### 需求分级与 PR 判断

每次接收到新需求时，按以下标准判断是否需要开 PR：

| 需求类型 | 是否开PR | 示例 |
|---------|---------|------|
| **新功能开发** | ✅ 必须 | 添加拖拽定位、新增按钮类型 |
| **架构重构** | ✅ 必须 | 模块拆分、API重构 |
| **UI/交互优化** | ✅ 必须 | 交互逻辑改变、新增设置项 |
| **Bug修复** | ❌ 可直接提交 | 修复崩溃、显示异常、逻辑错误 |
| **文档更新** | ❌ 可直接提交 | README、开发文档维护 |
| **代码微调** | ❌ 可直接提交 | 变量命名、注释补充、格式整理 |

### 开发流程

1. **需求分析** → 判断是否需要开 PR
2. **若需开 PR**：
   - 创建功能分支（命名如 `feature/hud-drag-position`）
   - 创建需求方案文档（存放于 `开发文档/` 目录）
   - 开发、测试、调试
   - 完成后创建 PR，等待用户确认合并
3. **跟进合并** — PR 创建后主动询问用户是否合并入 main
4. **若不需开 PR** → 直接在 main 分支完成并提交

### 合并确认机制

- PR 创建完成后，必须询问用户：「PR 已创建，是否合并入 main 分支？」
- 用户确认后才执行合并操作
- 合并后及时更新相关开发文档

### 自动打包机制

**触发时机**：每次功能完成或 Bug 修复合并到 main 后，自动打包分发版本。

**打包命令**：
```bash
python pack.py
```

**输出位置**：`releases/MMY_Blender_Toolkit_v0.5.0_20260521_1430.zip`

**命名格式**：`MMY_Blender_Toolkit_{版本号}_{日期时间}.zip`

**打包内容**：
- `mmy_toolkit/` 整个目录
- 排除：`__pycache__`、`.pyc`、用户自定义预设

**版本号来源**：自动读取 `mmy_toolkit/__init__.py` 的 `bl_info["version"]`

---

## 模块化开发文档管理

每次开发新功能或新需求时，按以下流程处理：

1. **分析模块归属** — 判断该功能属于哪个现有模块
2. **文档操作**：
   - 有对应模块 → 更新该模块的开发文档
   - 无对应模块 → 新建开发文档（命名清晰，如 `材质工具模块.md`）
3. **持续维护** — 后续围绕该模块的任何优化、调整、Bug修复，都要同步更新对应的开发文档

**目的**：确保开发文档与代码实现保持同步，方便追溯和查阅。

---

## 从错误中学到的（Blender API）

以下是实际开发中遇到的问题，记录下来避免重复犯错：

| 错误 | 原因 | 解决方案 |
|------|------|----------|
| Property 属性名以 `_` 开头 | Blender 不允许私有属性名 | 改用不带 `_` 的名称，或直接删除未使用的属性 |
| 自定义 Modal 菜单卡顿不实时 | draw_handler 绘制比标准菜单慢 | 使用 `bpy.ops.wm.call_menu` 调用标准 `bpy.types.Menu` |
| Modal 菜单弹出后立即消失 | 第一个事件仍是触发菜单的右键 PRESS | 添加 `ignore_first_rightmouse` 标志跳过首个右键事件 |
| 鼠标移出立即关闭菜单 | 菜单弹出时鼠标不在菜单区域 | 添加 `mouse_entered` 标志，只有鼠标曾进入过才允许移出关闭 |

**关键原则**：
- 标准 Blender 菜单 (`bpy.types.Menu`) 优先于自定义 draw_handler
- Modal Operator 要处理"触发事件"的边界情况
- 体验问题优先考虑原生方案

---

## 自动修复规则

修复问题时遵循以下原则：

- **最多尝试 3 次**，不要无限重试
- **不要尝试同一个修法两次**，失败后换思路
- **3 次都失败时**，说明尝试了什么、还有什么没修好，等待用户决策